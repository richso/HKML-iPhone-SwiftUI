...
class ForumWebviewController: UIViewController, WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler
{
    var parentView: ForumWebviewSwiftUIView?
    ...
    func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: @escaping ((WKNavigationActionPolicy) -> Void)) {
        ...
        if (navigationAction.navigationType == WKNavigationType.linkActivated) {
            if ((navigationAction.request.url?.absoluteString.lowercased().hasPrefix("facebookshare:"))!) {
                ...
                self.jumpManager!.fb_url = navigationAction.request.url!.absoluteString
                self.parentView!.selectTab(2)
        ...
        if (didReceive.name == "hkmlAppThumbnail") {
            ...
            self.webView?.configuration.websiteDataStore.httpCookieStore.getAllCookies({
                    (cookies) in
                        ...
                        SKPhotoBrowserOptions.sessionConfiguration.httpShouldSetCookies = false
                        SKPhotoBrowserOptions.sessionConfiguration.httpAdditionalHeaders =
                        [
                            "Cookie": cookie_str
                        ]
